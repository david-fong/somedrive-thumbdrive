...
def _get_removable_drives():
	...
	if platform.system() == "Linux":
		media = Path("/media", getpass.getuser())
		drives = [str(media / d) for d in os.listdir(str(media))]
...
def get_encrypted_drives():
	usb_list = _get_removable_drives()
	return [x for x in usb_list if _is_drive_encrypted(x)]


def get_unencrypted_drives():
	usb_list = _get_removable_drives()
	return [x for x in usb_list if not _is_drive_encrypted(x)]


def _is_drive_encrypted(root: str):
...
def load_encrypted_drive_new_user(root: str, password_input: str, user_email: str):
	...
	service = OpenDrive(drive_root=root, drive_id=drive_id, key=drive_key)
	return service
...
class OpenDrive:
	...
	def mount_fuse(self, mount_path):
		self.unmount_fuse()
		self.fuse = fusehandler.open_fuse(root=self.drive_root, mount=mount_path, key=self.key)
